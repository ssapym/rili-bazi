# 八字排盘系统字段参考手册

本文档详细说明 API 返回的所有 JSON 字段含义及数据结构。

## 命名规范

### 通用规则

- 通用英文术语使用英文，如 baseInfo、age、gender
- 命理专用术语使用拼音，如 tiangan、dizhi、nayin、shensha
- 组合概念也使用拼音，如 canggan、fuxing、xingyun

### 第一层模块

| 字段 | 类型 | 说明 |
|------|------|------|
| baseInfo | object | 基本信息，包含用户出生基本数据 |
| sizhu | object | 四柱信息，年、月、日、时 |
| chonghe | object | 刑冲合害关系 |
| nengliang | object | 能量分析 |
| geju | object | 格局分析 |
| dayun | object | 大运流年 |
| shensha | array | 神煞数组（包含在 sizhu 中） |

---

## baseInfo（基本信息）

### 数据结构

```json
{
  "baseInfo": {
    "solarBirthday": "1997年4月11日 10:00",
    "lunarBirthday": "丁丑三月初五丁巳时",
    "gender": "女性",
    "chineseZodiac": "丑 (牛)",
    "zodiac": "白羊座",
    "age": 28,
    "bazi": "丁丑 甲辰 癸未 丁巳",
    "baziArr": {
      "niangan": "丁",
      "nianzhi": "丑",
      "yuegan": "甲",
      "yuezhi": "辰",
      "rigan": "癸",
      "rizhi": "未",
      "shigan": "丁",
      "shizhi": "巳"
    }
  }
}
```

### 字段说明

| 字段 | 类型 | 说明 |
|------|------|------|
| solarBirthday | string | 公历生日，格式如「1997年4月11日 10:00」 |
| lunarBirthday | string | 农历生日，格式如「丁丑三月初五丁巳时」 |
| gender | string | 性别，「男性」或「女性」 |
| chineseZodiac | string | 生肖，格式如「丑 (牛)」 |
| zodiac | string | 星座，格式如「白羊座」 |
| age | number | 虚岁年龄 |
| bazi | string | 八字字符串，格式如「丁丑 甲辰 癸未 丁巳」 |
| baziArr | object | 八字数组对象 |

### baziArr 字段

| 字段 | 类型 | 说明 |
|------|------|------|
| niangan | string | 年干 |
| nianzhi | string | 年支 |
| yuegan | string | 月干 |
| yuezhi | string | 月支 |
| rigan | string | 日干 |
| rizhi | string | 日支 |
| shigan | string | 时干 |
| shizhi | string | 时支 |

---

## sizhu（四柱）

### 数据结构

```json
{
  "sizhu": {
    "nian": {
      "zhuxing": "偏财",
      "tiangan": "丁",
      "dizhi": "丑",
      "canggan": [
        { "ming": "己", "jibie": "本气", "shishen": "七杀", "fuxing": "比肩" },
        { "ming": "癸", "jibie": "中气", "shishen": "比肩", "fuxing": "食神" },
        { "ming": "辛", "jibie": "余气", "shishen": "偏印", "fuxing": "劫财" }
      ],
      "fuxing": "",
      "xingyun": "冠带",
      "zizuo": "墓",
      "kongwang": ["申", "酉"],
      "nayin": "涧下水",
      "shensha": ["天德合", "月德合", "福星"]
    },
    "yue": {...},
    "ri": {...},
    "shi": {...}
  }
}
```

### 字段说明

| 字段 | 类型 | 说明 |
|------|------|------|
| nian | object | 年柱 |
| yue | object | 月柱 |
| ri | object | 日柱 |
| shi | object | 时柱 |

### 四柱通用字段

| 字段 | 类型 | 说明 |
|------|------|------|
| zhuxing | string | 主星，该柱天干对应的十神 |
| tiangan | string | 天干 |
| dizhi | string | 地支 |
| canggan | array | 藏干数组 |
| fuxing | string | 副星（备用字段） |
| xingyun | string | 星运/地势 |
| zizuo | string | 自坐 |
| kongwang | array | 空亡数组 |
| nayin | string | 纳音 |
| shensha | array | 神煞数组 |

### canggan（藏干）

| 字段 | 类型 | 说明 |
|------|------|------|
| ming | string | 藏干名称 |
| jibie | string | 本气/中气/余气 |
| shishen | string | 十神（主星） |
| fuxing | string | 副星（地支对应的十神） |

---

## chonghe（刑冲合害关系）

### 数据结构

```json
{
  "chonghe": {
    "stems": [
      { "source": "年+月", "desc": "甲己合化土", "type": "合", "detailType": "天干五合" }
    ],
    "branches": [
      { "source": "年+月", "desc": "子丑合化土", "type": "合", "detailType": "地支六合" }
    ],
    "pillars": [
      { 
        "source": "年+月", 
        "desc": "甲子合己丑", 
        "type": "双合",
        "detailType": "双合",
        "details": "甲子与己丑"
      }
    ]
  }
}
```

### 字段说明

| 字段 | 类型 | 说明 |
|------|------|------|
| stems | array | 天干关系数组 |
| branches | array | 地支关系数组 |
| pillars | array | 干支组合关系数组 |

### 关系对象

| 字段 | 类型 | 说明 |
|------|------|------|
| source | string | 关系源柱位，如「年+月」 |
| desc | string | 关系描述，如「甲己合化土」 |
| type | string | **关系大类**（抽象分类），用于前端判断显示，如「合」、「冲」、「刑」、「害」、「破」、「绝」、「夹」、「克」、「双冲」、「双合」 |
| detailType | string | **关系细类**（具体类型），用于前端描述显示，如「天干五合」、「地支三合」、「半三合」等 |
| details | string | 详细说明（仅pillars字段），如「甲子与己丑」 |

### 关系类型说明

关系类型分为**大类**和**细类**两个层次：

- **大类（type）**：抽象分类，用于前端判断显示，如「合」、「冲」、「刑」等
- **细类（detailType）**：具体类型，用于前端描述显示，如「天干五合」、「地支三合」、「半三合」等

#### 大类分类

| 大类 | 包含的细类 | 关系类型编号 |
|------|-----------|------------|
| **冲** | 天干相冲、地支六冲 | 0, 2 |
| **合** | 天干五合、地支六合、地支三合、半三合、拱合、暗合 | 1, 6, 7, 8, 9, 12 |
| **会** | 地支三会、半三会 | 10, 11 |
| **刑** | 地支三刑（三刑）、地支相刑、地支自刑 | 3, 4, 5 |
| **害** | 地支六害 | 13 |
| **破** | 地支六破 | 16 |
| **绝** | 地支四绝 | 20 |
| **夹** | 夹 | 14 |
| **克** | 天干相克 | 15 |
| **双冲** | 双冲、天克地刑 | 17, 18 |
| **双合** | 双合 | 19 |

#### 细类说明

| 大类 | 细类 | 说明 | 示例 |
|------|------|------|------|
| **合** | 天干五合 | 两干相合，化气生财 | 甲己合化土、乙庚合化金 |
| **合** | 地支六合 | 两支相合，关系密切 | 子丑合土、寅亥合木 |
| **合** | 地支三合 | 三支合局，力量强大 | 申子辰合水、亥卯未合木 |
| **合** | 半三合 | 两支半合，力量次之 | 申子半合水、亥卯半合木 |
| **合** | 拱合 | 两支拱合，推导第三支 | 申辰拱合子、亥未拱合卯 |
| **合** | 暗合 | 两支暗合，暗中相合 | 卯申暗合、午亥暗合 |
| **会** | 地支三会 | 三支会局，季节性合局 | 寅卯辰会木、巳午未会火 |
| **会** | 半三会 | 两支半会，季节性半合 | 亥子半会水、寅卯半会木 |
| **冲** | 天干相冲 | 两干相冲，对立冲突 | 甲庚冲、乙辛冲 |
| **冲** | 地支六冲 | 两支相冲，冲突对立 | 子午冲、丑未冲 |
| **刑** | 地支三刑（三刑） | 三支相刑，三刑并立 | 寅巳申三刑、丑戌未三刑 |
| **刑** | 地支相刑 | 两支相刑，互相刑克 | 寅巳相刑、巳申相刑、子卯相刑 |
| **刑** | 地支自刑 | 支自相刑，自我刑克 | 辰辰自刑、午午自刑 |
| **害** | 地支六害 | 两支相害，暗中损害 | 子未害、丑午害 |
| **破** | 地支六破 | 两支相破，破坏关系 | 子酉破、卯午破 |
| **绝** | 地支四绝 | 两支相绝，绝灭关系 | 寅酉绝、卯申绝 |
| **夹** | 夹 | 两支夹合，夹出第三支 | 子寅夹丑、丑卯夹寅 |
| **克** | 天干相克 | 五行相克关系，克制对方 | 甲克戊、丙克庚 |
| **双冲** | 双冲 | 天干相冲且地支也相冲 | 甲子冲庚午、乙丑冲辛未 |
| **双冲** | 天克地刑 | 天干相克且地支自刑 | 乙亥克己亥、戊辰克壬辰 |
| **双合** | 双合 | 天干相合且地支也相合 | 甲子合己丑、丙寅合辛亥 |

### 天干相克关系

天干相克遵循五行相克原理：木克土、火克金、土克水、金克木、水克火。

| 相克关系 | 五行原理 | 说明 |
|----------|----------|------|
| 甲克戊 | 木克土 | 甲木克制戊土 |
| 乙克己 | 木克土 | 乙木克制己土 |
| 丙克庚 | 火克金 | 丙火克制庚金 |
| 丁克辛 | 火克金 | 丁火克制辛金 |
| 戊克壬 | 土克水 | 戊土克制壬水 |
| 己克癸 | 土克水 | 己土克制癸水 |

### 地支六破关系

地支六破是指地支之间存在的破坏关系，通常表示关系的破裂或破坏。

| 六破关系 | 说明 |
|----------|------|
| 子酉破 | 子水与酉金相破，金水相破 |
| 卯午破 | 卯木与午火相破，木火相破 |
| 辰丑破 | 辰土与丑土相破，土土相破 |
| 未戌破 | 未土与戌土相破，土土相破 |
| 寅亥破 | 寅木与亥水相破，木水相破 |
| 申巳破 | 申金与巳火相破，金火相破 |

### 地支四绝关系

地支四绝是指地支之间存在的绝灭关系，通常表示关系的绝灭或断绝。

| 四绝关系 | 说明 |
|----------|------|
| 寅酉绝 | 寅木与酉金相绝，金克木绝 |
| 卯申绝 | 卯木与申金相绝，金克木绝 |
| 午亥绝 | 午火与亥水相绝，水克火绝 |
| 子巳绝 | 子水与巳火相绝，水克火绝 |

### 干支组合关系

干支组合关系是指两个完整的干支柱之间同时存在天干关系和地支关系的情况。

#### 双冲关系（60组）

双冲是指两个干支柱之间，天干相冲且地支也相冲，这是最强的冲克关系。

**甲行双冲（6个）**
| 双冲关系 | 说明 |
|----------|------|
| 甲子冲庚午 | 甲庚冲（天干）+ 子午冲（地支） |
| 甲戌冲庚辰 | 甲庚冲（天干）+ 辰戌冲（地支） |
| 甲申冲庚寅 | 甲庚冲（天干）+ 寅申冲（地支） |
| 甲午冲庚子 | 甲庚冲（天干）+ 子午冲（地支） |
| 甲辰冲庚戌 | 甲庚冲（天干）+ 辰戌冲（地支） |
| 甲寅冲庚申 | 甲庚冲（天干）+ 寅申冲（地支） |

**乙行双冲（6个）**
| 双冲关系 | 说明 |
|----------|------|
| 乙丑冲辛未 | 乙辛冲（天干）+ 丑未冲（地支） |
| 乙亥冲辛巳 | 乙辛冲（天干）+ 巳亥冲（地支） |
| 乙酉冲辛卯 | 乙辛冲（天干）+ 卯酉冲（地支） |
| 乙未冲辛丑 | 乙辛冲（天干）+ 丑未冲（地支） |
| 乙巳冲辛亥 | 乙辛冲（天干）+ 巳亥冲（地支） |
| 乙卯冲辛酉 | 乙辛冲（天干）+ 卯酉冲（地支） |

**丙行双冲（6个）**
| 双冲关系 | 说明 |
|----------|------|
| 丙寅冲癸酉 | 丙壬冲（天干）+ 寅申冲（地支） |
| 丙子冲壬午 | 丙壬冲（天干）+ 子午冲（地支） |
| 丙戌冲壬辰 | 丙壬冲（天干）+ 辰戌冲（地支） |
| 丙申冲壬寅 | 丙壬冲（天干）+ 寅申冲（地支） |
| 丙午冲壬子 | 丙壬冲（天干）+ 子午冲（地支） |
| 丙辰冲壬戌 | 丙壬冲（天干）+ 辰戌冲（地支） |

**丁行双冲（6个）**
| 双冲关系 | 说明 |
|----------|------|
| 丁卯冲壬申 | 丁癸冲（天干）+ 卯酉冲（地支） |
| 丁丑冲癸未 | 丁癸冲（天干）+ 丑未冲（地支） |
| 丁亥冲癸巳 | 丁癸冲（天干）+ 巳亥冲（地支） |
| 丁酉冲癸卯 | 丁癸冲（天干）+ 卯酉冲（地支） |
| 丁未冲癸丑 | 丁癸冲（天干）+ 丑未冲（地支） |
| 丁巳冲癸亥 | 丁癸冲（天干）+ 巳亥冲（地支） |

**戊行双冲（6个）**
| 双冲关系 | 说明 |
|----------|------|
| 戊辰冲甲戌 | 戊甲冲（天干）+ 辰戌冲（地支） |
| 戊寅冲甲申 | 戊甲冲（天干）+ 寅申冲（地支） |
| 戊午冲甲子 | 戊甲冲（天干）+ 子午冲（地支） |
| 戊戌冲甲辰 | 戊甲冲（天干）+ 辰戌冲（地支） |
| 戊申冲甲寅 | 戊甲冲（天干）+ 寅申冲（地支） |
| 戊子冲甲午 | 戊甲冲（天干）+ 子午冲（地支） |

**己行双冲（6个）**
| 双冲关系 | 说明 |
|----------|------|
| 己巳冲乙亥 | 己乙冲（天干）+ 巳亥冲（地支） |
| 己卯冲乙酉 | 己乙冲（天干）+ 卯酉冲（地支） |
| 己丑冲乙未 | 己乙冲（天干）+ 丑未冲（地支） |
| 己亥冲乙巳 | 己乙冲（天干）+ 巳亥冲（地支） |
| 己酉冲乙卯 | 己乙冲（天干）+ 卯酉冲（地支） |
| 己未冲乙丑 | 己乙冲（天干）+ 丑未冲（地支） |

**庚行双冲（6个）**
| 双冲关系 | 说明 |
|----------|------|
| 庚午冲丙子 | 庚丙冲（天干）+ 子午冲（地支） |
| 庚辰冲丙戌 | 庚丙冲（天干）+ 辰戌冲（地支） |
| 庚寅冲丙申 | 庚丙冲（天干）+ 寅申冲（地支） |
| 庚子冲丙午 | 庚丙冲（天干）+ 子午冲（地支） |
| 庚戌冲丙辰 | 庚丙冲（天干）+ 辰戌冲（地支） |
| 庚申冲丙寅 | 庚丙冲（天干）+ 寅申冲（地支） |

**辛行双冲（6个）**
| 双冲关系 | 说明 |
|----------|------|
| 辛未冲丁丑 | 辛丁冲（天干）+ 丑未冲（地支） |
| 辛巳冲丁亥 | 辛丁冲（天干）+ 巳亥冲（地支） |
| 辛卯冲丁酉 | 辛丁冲（天干）+ 卯酉冲（地支） |
| 辛丑冲丁未 | 辛丁冲（天干）+ 丑未冲（地支） |
| 辛亥冲丁巳 | 辛丁冲（天干）+ 巳亥冲（地支） |
| 辛酉冲丁卯 | 辛丁冲（天干）+ 卯酉冲（地支） |

**壬行双冲（6个）**
| 双冲关系 | 说明 |
|----------|------|
| 壬申冲戊寅 | 壬戊冲（天干）+ 寅申冲（地支） |
| 壬午冲戊子 | 壬戊冲（天干）+ 子午冲（地支） |
| 壬辰冲戊戌 | 壬戊冲（天干）+ 辰戌冲（地支） |
| 壬寅冲戊申 | 壬戊冲（天干）+ 寅申冲（地支） |
| 壬子冲戊午 | 壬戊冲（天干）+ 子午冲（地支） |
| 壬戌冲戊辰 | 壬戊冲（天干）+ 辰戌冲（地支） |

**癸行双冲（6个）**
| 双冲关系 | 说明 |
|----------|------|
| 癸酉冲己卯 | 癸己冲（天干）+ 卯酉冲（地支） |
| 癸未冲己丑 | 癸己冲（天干）+ 丑未冲（地支） |
| 癸巳冲己亥 | 癸己冲（天干）+ 巳亥冲（地支） |
| 癸卯冲己酉 | 癸己冲（天干）+ 卯酉冲（地支） |
| 癸丑冲己未 | 癸己冲（天干）+ 丑未冲（地支） |
| 癸亥冲己巳 | 癸己冲（天干）+ 巳亥冲（地支） |

#### 天克地刑（自刑）关系（20组）

天克地刑是指两个干支柱之间，天干相克且地支自刑，这是一种特殊的冲克关系。

**乙亥系列（5个）**
| 天克地刑关系 | 说明 |
|--------------|------|
| 乙亥克己亥 | 乙己克（天干）+ 亥亥自刑（地支） |
| 乙亥克辛亥 | 乙辛克（天干）+ 亥亥自刑（地支） |
| 丁亥克辛亥 | 丁辛克（天干）+ 亥亥自刑（地支） |
| 丁亥克癸亥 | 丁癸克（天干）+ 亥亥自刑（地支） |
| 己亥克癸亥 | 己癸克（天干）+ 亥亥自刑（地支） |

**戊辰系列（5个）**
| 天克地刑关系 | 说明 |
|--------------|------|
| 戊辰克壬辰 | 戊壬克（天干）+ 辰辰自刑（地支） |
| 戊辰克甲辰 | 戊甲克（天干）+ 辰辰自刑（地支） |
| 庚辰克甲辰 | 庚甲克（天干）+ 辰辰自刑（地支） |
| 庚辰克丙辰 | 庚丙克（天干）+ 辰辰自刑（地支） |
| 壬辰克丙辰 | 壬丙克（天干）+ 辰辰自刑（地支） |

**庚午系列（5个）**
| 天克地刑关系 | 说明 |
|--------------|------|
| 庚午克丙午 | 庚丙克（天干）+ 午午自刑（地支） |
| 庚午克甲午 | 庚甲克（天干）+ 午午自刑（地支） |
| 壬午克丙午 | 壬丙克（天干）+ 午午自刑（地支） |
| 壬午克戊午 | 壬戊克（天干）+ 午午自刑（地支） |
| 甲午克戊午 | 甲戊克（天干）+ 午午自刑（地支） |

**癸酉系列（5个）**
| 天克地刑关系 | 说明 |
|--------------|------|
| 癸酉克己酉 | 癸己克（天干）+ 酉酉自刑（地支） |
| 癸酉克丁酉 | 癸丁克（天干）+ 酉酉自刑（地支） |
| 乙酉克己酉 | 乙己克（天干）+ 酉酉自刑（地支） |
| 乙酉克辛酉 | 乙辛克（天干）+ 酉酉自刑（地支） |
| 丁酉克辛酉 | 丁辛克（天干）+ 酉酉自刑（地支） |

#### 双合关系（30组）

双合是指两个干支柱之间，天干相合且地支也相合，这是最强的合化关系。

**甲己双合（6个）**
| 双合关系 | 说明 |
|----------|------|
| 甲子合己丑 | 甲己合（天干）+ 子丑合（地支） |
| 甲寅合己亥 | 甲己合（天干）+ 寅亥合（地支） |
| 甲辰合己酉 | 甲己合（天干）+ 辰酉合（地支） |
| 甲午合己未 | 甲己合（天干）+ 午未合（地支） |
| 甲申合己巳 | 甲己合（天干）+ 申巳合（地支） |
| 甲戌合己卯 | 甲己合（天干）+ 戌卯合（地支） |

**丙辛双合（6个）**
| 双合关系 | 说明 |
|----------|------|
| 丙子合辛丑 | 丙辛合（天干）+ 子丑合（地支） |
| 丙寅合辛亥 | 丙辛合（天干）+ 寅亥合（地支） |
| 丙辰合辛酉 | 丙辛合（天干）+ 辰酉合（地支） |
| 丙午合辛未 | 丙辛合（天干）+ 午未合（地支） |
| 丙申合辛巳 | 丙辛合（天干）+ 申巳合（地支） |
| 丙戌合辛卯 | 丙辛合（天干）+ 戌卯合（地支） |

**戊癸双合（6个）**
| 双合关系 | 说明 |
|----------|------|
| 戊子合癸丑 | 戊癸合（天干）+ 子丑合（地支） |
| 戊寅合癸亥 | 戊癸合（天干）+ 寅亥合（地支） |
| 戊辰合癸酉 | 戊癸合（天干）+ 辰酉合（地支） |
| 戊午合癸未 | 戊癸合（天干）+ 午未合（地支） |
| 戊申合癸巳 | 戊癸合（天干）+ 申巳合（地支） |
| 戊戌合癸卯 | 戊癸合（天干）+ 戌卯合（地支） |

**庚乙双合（6个）**
| 双合关系 | 说明 |
|----------|------|
| 庚子合乙丑 | 庚乙合（天干）+ 子丑合（地支） |
| 庚寅合乙亥 | 庚乙合（天干）+ 寅亥合（地支） |
| 庚辰合乙酉 | 庚乙合（天干）+ 辰酉合（地支） |
| 庚午合乙未 | 庚乙合（天干）+ 午未合（地支） |
| 庚申合乙巳 | 庚乙合（天干）+ 申巳合（地支） |
| 庚戌合乙卯 | 庚乙合（天干）+ 戌卯合（地支） |

**壬丁双合（6个）**
| 双合关系 | 说明 |
|----------|------|
| 壬子合丁丑 | 壬丁合（天干）+ 子丑合（地支） |
| 壬寅合丁亥 | 壬丁合（天干）+ 寅亥合（地支） |
| 壬辰合丁酉 | 壬丁合（天干）+ 辰酉合（地支） |
| 壬午合丁未 | 壬丁合（天干）+ 午未合（地支） |
| 壬申合丁巳 | 壬丁合（天干）+ 申巳合（地支） |
| 壬戌合丁卯 | 壬丁合（天干）+ 戌卯合（地支） |

---

## nengliang（能量分析）

### 数据结构

```json
{
  "nengliang": {
    "wuxing": {
      "elements": [
        { "id": "wood", "name": "木", "score": 6.5, "level": "囚", "logs": [...] }
      ],
      "totalScore": 67.8,
      "balance": 0.55
    },
    "shishen": {
      "zonggeshu": 16,
      "byCount": [
        { "ming": "偏财", "geshu": 3, "baifengbi": 18.8 }
      ],
      "byEnergy": [
        { "ming": "正财", "fenshu": 27, "baifengbi": 18 }
      ]
    }
  }
}
```

### wuxing（五行能量）

| 字段 | 类型 | 说明 |
|------|------|------|
| elements | array | 五行元素数组 |
| totalScore | number | 总分 |
| balance | number | 平衡指数 |
| count | object | 五行数量统计（木/火/土/金/水） |
| missing | array | 缺失的五行列表 |
| isComplete | boolean | 五行是否齐全 |

### wuxing.elements（五行元素）

| 字段 | 类型 | 说明 |
|------|------|------|
| id | string | 五行 ID：wood/fire/earth/metal/water |
| name | string | 五行名称：木/火/土/金/水 |
| score | number | 得分 |
| level | string | 旺衰程度：旺、相、休、囚、死 |
| logs | array | 计算日志 |

### wuxing.count（五行数量统计）

| 字段 | 类型 | 说明 |
|------|------|------|
| wood | number | 木的个数 |
| fire | number | 火的个数 |
| earth | number | 土的个数 |
| metal | number | 金的个数 |
| water | number | 水的个数 |

### wuxing.missing（缺失五行）

| 字段 | 类型 | 说明 |
|------|------|------|
| - | array | 缺失的五行列表（如：['金', '土']） |

### wuxing.isComplete（五行是否齐全）

| 字段 | 类型 | 说明 |
|------|------|------|
| - | boolean | 五行是否齐全（true=齐全，false=缺失） |

### shishen（十神能量）

| 字段 | 类型 | 说明 |
|------|------|------|
| zonggeshu | number | 总个数 |
| byCount | array | 按个数统计 |
| byEnergy | array | 按能量统计 |

### byCount / byEnergy 统计项

| 字段 | 类型 | 说明 |
|------|------|------|
| ming | string | 十神名称 |
| geshu | number | 个数（仅 byCount） |
| fenshu | number | 分数（仅 byEnergy） |
| baifengbi | number | 百分比 |

### 十神对照表

| 日干 | 比肩 | 劫财 | 食神 | 伤官 | 偏财 | 正财 | 七杀 | 正官 | 偏印 | 正印 |
|------|------|------|------|------|------|------|------|------|------|------|
| 甲 | 甲 | 乙 | 丙 | 丁 | 戊 | 己 | 庚 | 辛 | 壬 | 癸 |
| 乙 | 乙 | 甲 | 丁 | 丙 | 己 | 戊 | 辛 | 庚 | 癸 | 壬 |
| 丙 | 丙 | 丁 | 戊 | 己 | 庚 | 辛 | 壬 | 癸 | 甲 | 乙 |
| 丁 | 丁 | 丙 | 己 | 戊 | 辛 | 庚 | 癸 | 壬 | 乙 | 甲 |
| 戊 | 戊 | 己 | 庚 | 辛 | 壬 | 癸 | 甲 | 乙 | 丙 | 丁 |
| 己 | 己 | 戊 | 辛 | 庚 | 癸 | 壬 | 乙 | 甲 | 丁 | 丙 |
| 庚 | 庚 | 辛 | 壬 | 癸 | 甲 | 乙 | 丙 | 丁 | 戊 | 己 |
| 辛 | 辛 | 庚 | 癸 | 壬 | 乙 | 甲 | 丁 | 丙 | 己 | 戊 |
| 壬 | 壬 | 癸 | 甲 | 乙 | 丙 | 丁 | 戊 | 己 | 庚 | 辛 |
| 癸 | 癸 | 壬 | 乙 | 甲 | 丁 | 丙 | 己 | 戊 | 辛 | 庚 |

### 五行特性

| 五行 | 代表颜色 | 性格特征 | 职业方向 |
|------|----------|----------|----------|
| 木 | 绿色、青色 | 仁慈、善良、有生发性 | 教育、文化、设计 |
| 火 | 红色、紫色 | 热情、开朗、有活力 | 餐饮、娱乐、演艺 |
| 土 | 黄色、棕色 | 稳重、厚道、有诚信 | 房地产、建筑、农业 |
| 金 | 白色、金色 | 果断、刚毅、有原则 | 金融、法律、军警 |
| 水 | 蓝色、黑色 | 聪明、灵活、有智慧 | 贸易、咨询、科技 |

---

## geju（格局分析）

### 数据结构

```json
{
  "geju": {
    "rizhu": "癸，水命人",
    "geju": "普通格局",
    "shuoming": "",
    "tiaohou": "先丙后辛，甲木次之。",
    "xiyong": ["金", "木"],
    "jihui": ["火", "土"],
    "jianyi": "此命局无明显格局，以普通命局论。日主平衡。取金、木为用神，火、土为忌神。先丙后辛，甲木次之。",
    "qiangruo": {
      "chengdu": "平衡",
      "fenshu": 0
    }
  }
}
```

### 字段说明

| 字段 | 类型 | 说明 |
|------|------|------|
| rizhu | string | 日主描述，格式如「癸，水命人」 |
| geju | string | 格局名称 |
| shuoming | string | 格局说明 |
| tiaohou | string | 调候用神 |
| xiyong | array | 喜用元素数组 |
| jihui | array | 忌讳元素数组 |
| jianyi | string | 建议 |
| qiangruo | object | 强弱分析 |

### qiangruo（强弱分析）

| 字段 | 类型 | 说明 |
|------|------|------|
| chengdu | string | 程度描述：太弱、偏弱、平衡、偏旺、旺、从旺 |
| fenshu | number | 分数 |

### 身强程度评分标准

| 程度 | 分数范围 | 含义 |
|------|----------|------|
| 太弱 | 0-20 | 日主极弱，需依靠他人 |
| 偏弱 | 21-40 | 日主较弱，需生扶 |
| 平衡 | 41-60 | 日主平衡，运势平稳 |
| 偏旺 | 61-80 | 日主较强，需克制 |
| 旺 | 81-90 | 日主极旺，顺其势而为 |
| 从旺 | 91-100 | 日主极强，宜顺势发展 |

---

## dayun（大运流年）

### 数据结构

```json
{
  "dayun": {
    "taiyuan": "乙未(沙中金)",
    "taixi": "戊午(天上火)",
    "minggong": "戊申(大驿土)",
    "shengong": "庚戌(钗钏金)",
    "qiyun": "8年1个月15天12时52分 (2005年5月26日  23:05:00后起运)",
    "dayunCount": 11,
    "dayun": [
      {
        "id": 0,
        "name": "童限",
        "startYear": 1997,
        "endYear": 2005,
        "startAge": 1,
        "endAge": 8,
        "shishen": "伤官"
      },
      {
        "id": 1,
        "name": "大运1",
        "startYear": 2013,
        "endYear": 2022,
        "startAge": 9,
        "endAge": 18,
        "shishen": "食神"
      }
    ]
  }
}
```

### 字段说明

| 字段 | 类型 | 说明 |
|------|------|------|
| taiyuan | string | 胎元 |
| taixi | string | 胎息 |
| minggong | string | 命宫 |
| shengong | string | 身宫 |
| qiyun | string | 起运信息 |
| dayunCount | number | 大运数量 |
| dayun | array | 大运数组 |

### dayun（单元）

| 字段 | 类型 | 说明 |
|------|------|------|
| id | number | 编号（0=童限，1-10=大运） |
| name | string | 名称 |
| startYear | number | 起始年代 |
| endYear | number | 结束年代 |
| startAge | number | 起始年龄 |
| endAge | number | 结束年龄 |
| shishen | string | 十神 |

---

## shensha（神煞）

神煞信息包含在四柱的 shensha 字段中。

### 常见神煞

| 神煞 | 五行 | 含义与作用 |
|------|------|-----------|
| 天乙贵人 | 土 | 最吉利的神煞，能化解危难，遇难成祥 |
| 太极贵人 | 水 | 主聪明智慧，爱好学习，有艺术天赋 |
| 天德贵人 | 土 | 逢凶化吉，贵人相助，消灾解厄 |
| 月德贵人 | 土 | 与天德类似，但影响力较小 |
| 文昌星 | 木 | 主学业功名，利于考试、升学 |
| 驿马星 | 火 | 主变动迁移，利于出行、搬家 |
| 华盖星 | 土 | 主艺术才华，喜神秘学 |
| 将星 | 金 | 主领导能力，有组织管理才能 |
| 红鸾星 | 火 | 主婚姻喜庆，桃花运 |
| 天喜星 | 水 | 主喜悦之事，有意外惊喜 |
| 桃花星 | 水 | 主感情运势，易有异性缘 |
| 阳刃 | 金 | 主个性刚强，易有意外伤害 |
| 长生 | 水 | 主生命力旺盛，有创造力 |
| 墓 | 土 | 主收藏、储藏，有积蓄能力 |
| 帝旺 | 木 | 主运势旺盛，有权威地位 |

---

## nayin（纳音）

纳音信息包含在四柱的 nayin 字段中。

### 纳音对照表

| 纳音 | 对应年份 | 五行 | 特性 |
|------|----------|------|------|
| 海中金 | 甲子、乙丑 | 金 | 深沉内敛，有贵人相助 |
| 炉中火 | 丙寅、丁卯 | 火 | 热情积极，有艺术天赋 |
| 大林木 | 戊辰、己巳 | 木 | 成长性好，有发展空间 |
| 路旁土 | 庚午、辛未 | 土 | 稳重踏实，有积蓄能力 |
| 剑锋金 | 壬申、癸酉 | 金 | 果断刚强，有领导力 |
| 山头火 | 甲戌、乙亥 | 火 | 热情大方，有创造力 |
| 涧下水 | 丙子、丁丑 | 水 | 聪明灵活，有智慧 |
| 城头土 | 戊寅、己卯 | 土 | 稳重厚道，有诚信 |
| 白蜡金 | 庚辰、辛巳 | 金 | 优雅精致，有品味 |
| 杨柳木 | 壬午、癸未 | 木 | 柔和灵活，适应性强 |
| 井泉水 | 甲申、乙酉 | 水 | 源源不断，有创造力 |
| 屋上土 | 丙戌、丁亥 | 土 | 有靠山，有贵人相助 |
| 霹雳火 | 戊子、己丑 | 火 | 刚烈威猛，有爆发力 |
| 松柏木 | 庚寅、辛卯 | 木 | 坚韧不拔，有毅力 |
| 长流水 | 壬辰、癸巳 | 水 | 财运亨通，有流动资产 |
| 砂中金 | 甲午、乙未 | 金 | 潜力股，有发展空间 |
| 山下火 | 丙申、丁酉 | 火 | 热情开朗，有感染力 |
| 平地木 | 庚子、辛丑 | 木 | 稳步发展，有积累 |
| 壁上土 | 壬寅、癸卯 | 土 | 有靠山，有贵人 |
| 金箔金 | 甲辰、乙巳 | 金 | 表面光鲜，有虚荣心 |
| 覆灯火 | 丙午、丁未 | 火 | 温柔细腻，有艺术天赋 |
| 天河水 | 戊申、己酉 | 水 | 财运好，有意外之财 |
| 大驿土 | 庚戌、辛亥 | 土 | 稳重踏实，有积蓄 |

---

## trueSolarInfo（真太阳时信息）

当 useTrueSolar=true 时返回此字段。

### 数据结构

```json
{
  "trueSolarInfo": {
    "inputHour": 10,
    "inputMinute": 0,
    "longitude": 116.42,
    "equationOfTime": -1.32,
    "timezoneOffset": -0.24,
    "adjustedHour": 10,
    "adjustedMinute": 13
  }
}
```

### 字段说明

| 字段 | 类型 | 说明 |
|------|------|------|
| inputHour | int | 原始输入时辰 |
| inputMinute | int | 原始输入分钟 |
| longitude | float | 地点经度 |
| equationOfTime | float | 时差方程值（分钟） |
| timezoneOffset | float | 时区偏移（小时） |
| adjustedHour | int | 调整后时辰 |
| adjustedMinute | int | 调整后分钟 |

---

## 天干地支对照表

### 天干

| 天干 | 五行 | 天干 | 五行 |
|------|------|------|------|
| 甲 | 木 | 己 | 土 |
| 乙 | 木 | 庚 | 金 |
| 丙 | 火 | 辛 | 金 |
| 丁 | 火 | 壬 | 水 |
| 戊 | 土 | 癸 | 水 |

### 地支

| 地支 | 五行 | 地支 | 五行 |
|------|------|------|------|
| 子 | 水 | 午 | 火 |
| 丑 | 土 | 未 | 土 |
| 寅 | 木 | 申 | 金 |
| 卯 | 木 | 酉 | 金 |
| 辰 | 土 | 戌 | 土 |
| 巳 | 火 | 亥 | 水 |

### 地支藏干

| 地支 | 藏干 | 地支 | 藏干 |
|------|------|------|------|
| 子 | 癸 | 午 | 丁、己 |
| 丑 | 己、癸、辛 | 未 | 己、丁、乙 |
| 寅 | 甲、丙、戊 | 申 | 庚、壬、戊 |
| 卯 | 乙 | 酉 | 辛 |
| 辰 | 戊、癸、乙 | 戌 | 戊、辛、丁 |
| 巳 | 丙、戊、庚 | 亥 | 壬、甲 |

---

## 时辰对照表

| 时辰 | 时间段 | 时辰 | 时间段 |
|------|--------|------|--------|
| 子时 | 23:00-00:59 | 午时 | 11:00-12:59 |
| 丑时 | 01:00-02:59 | 未时 | 13:00-14:59 |
| 寅时 | 03:00-04:59 | 申时 | 15:00-16:59 |
| 卯时 | 05:00-06:59 | 酉时 | 17:00-18:59 |
| 辰时 | 07:00-08:59 | 戌时 | 19:00-20:59 |
| 巳时 | 09:00-10:59 | 亥时 | 21:00-22:59 |
