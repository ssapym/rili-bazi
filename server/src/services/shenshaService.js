 ...
 * 神煞计算服务
 * 
 * 参考SPA端paipan.js/shensha.js的注释和逻辑移植
 * 
 * 神煞是中国传统命理学中的重要概念，
 * 包括天乙贵人、太乙、太极、天德、月德等众多神煞。
 * 
 * 神煞分类：
 * 1. 吉神：天乙贵人、太乙贵人、太极贵人、天德、月德、德秀、福星、文昌等
 * 2. 凶神：驿马、灾煞、岁破、月破等
 * 3. 中性神煞：华盖、将星、禄神等（吉凶看组合）
 * 
 * 神煞查询规则：
 * - 根据年柱、月柱、日柱、时柱分别查询
 * - 有些神煞只查特定柱（如：学堂、词馆只查年柱）
 * - 有些神煞根据性别判断（如：红艳煞）
 * 
 * 来源参考：
 * - paipan.js 中的神煞计算逻辑
 * - shensha.js 中的神煞定义和查询规则
 */
...
const { SixtyCycle } = require('tyme4ts');
...
// API封装
function calculateShenSha(pillars, isMan = true) {
  const bazi = [
    pillars.year.heavenStem, pillars.year.earthBranch,
    pillars.month.heavenStem, pillars.month.earthBranch,
    pillars.day.heavenStem, pillars.day.earthBranch,
    pillars.hour.heavenStem, pillars.hour.earthBranch
  ];
  
  const sixtyCycle = SixtyCycle.fromName(pillars.year.heavenStem + pillars.year.earthBranch);
  const niannayin = sixtyCycle.getSound().getName();

  return {
    year: queryShenSha(pillars.year.heavenStem + pillars.year.earthBranch, bazi, isMan, 1, niannayin),
    month: queryShenSha(pillars.month.heavenStem + pillars.month.earthBranch, bazi, isMan, 2, niannayin),
    day: queryShenSha(pillars.day.heavenStem + pillars.day.earthBranch, bazi, isMan, 3, niannayin),
    hour: queryShenSha(pillars.hour.heavenStem + pillars.hour.earthBranch, bazi, isMan, 4, niannayin)
  };
}
