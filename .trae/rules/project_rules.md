...
临时测试时可以在tmp目录下创建临时文件，测试完成后及时删除，不要保留到下一次测试；
功能测试脚本在test目录下，运行命令为node index.js，具体说明看docs目录下的TEST_GUIDE.md文件；
生成的代码都要有完整的注释，包括函数、类、模块等，也要有必要的说明，方便其他开发者理解和使用；
当一个需求完整实现后，要及时更新docs目录下的说明文件，包括功能说明、接口文档、使用示例、ChangeLog等；